...
const TenderManagement: React.FC = () => {
  ...
  const availableRequests = requests.filter(
    (r) => r.status === "Approved" && !r.tenderId
  );
  ...
  const generateSOW = async () => {
    ...
    const selectedAssetsSpecs = selectedRequestIds
      .map((reqId) => {
    ...
    try {
      const prompt = `Buat deskripsi Lingkup Kerja (Scope of Work) profesional untuk paket tender hulu migas.
        ...
        3. Standar CSMS MIGAS (High Risk).
      ...
      if (process.env.API_KEY) {
        ...
        setTenderDesc(
          `Lingkup kerja mencakup penyediaan, pengoperasian, dan pemeliharaan ${
            selectedAssetsSpecs.length
          } unit aset penunjang operasi hulu migas, antara lain: ${selectedAssetsSpecs.join(
            ", "
          )}. Kontraktor wajib memastikan seluruh unit memiliki sertifikasi kelas (BKI/IACS) yang aktif, memenuhi standar keselamatan migas (CSMS), serta mematuhi ketentuan TKDN sesuai PTK-007 Rev.05. Unit harus siap dimobilisasi sesuai jadwal operasi yang ditetapkan.`
        );
      }
      setTenderTitle(
        `Pengadaan Jasa Sewa ${
          selectedAssetsSpecs.length
        } Unit Aset Hulu Migas - ${new Date().getFullYear()}`
      );
  ...
  const handleCreateTender = () => {
    const newTender: Tender = {
      id: `TDR-${new Date().getFullYear()}-${Math.floor(
        Math.random() * 10000
      )}`,
      name: tenderTitle,
      ...
      if (process.env.API_KEY) {
        ...
        const prompt = `Evaluasi penawaran tender berikut berdasarkan PTK-007:
           ...
           ${selectedTender.bids
             ?.map(
               (b) =>
                 `- ${b.vendorName}: IDR ${b.bidAmount} (Teknis: ${b.complianceScore})`
             )
             .join("\n")}
        ...
        await new Promise((r) => setTimeout(r, 1500));
        const bestBid = selectedTender.bids?.sort(
          (a, b) => b.complianceScore! - a.complianceScore!
        )[0]; // Simple mock logic
        setAiContent(
          `**Laporan Evaluasi Tender**\n\nMetode Evaluasi: Kualitas & Biaya.\n\nRekomendasi Pemenang: **${
            bestBid?.vendorName || "N/A"
          }**\n\n**Analisis:**\n1. Vendor memiliki skor teknis tertinggi (${
            bestBid?.complianceScore
          }).\n2. Harga penawaran berada dalam batas kewajaran HPS.\n3. Status administrasi CIVD valid.`
        );
      }
    } catch (e) {
      setAiContent("Error evaluasi.");
    } finally {
      setIsGenerating(false);
    }
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">
            Manajemen Paket Tender
          </h1>
          <p className="text-slate-500 text-sm mt-1">
            Pengelolaan paket lelang dan pembukaan sampul penawaran.
          </p>
        </div>
        <button
          onClick={() => setIsCreateModalOpen(true)}
          className="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-bold shadow-sm hover:bg-indigo-700 transition-colors flex items-center gap-2"
        >
          ...
          {filteredTenders.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64 text-slate-400">
              <div className="w-16 h-16 bg-slate-100 rounded-full flex items-center justify-center mb-4">
                <Gavel size={32} className="opacity-50" />
              </div>
              <p className="text-sm font-medium">
                Tidak ada paket tender ditemukan.
              </p>
            </div>
          ) : (
            filteredTenders.map((tender) => {
              ...
              return (
                <div
                  ...
                  className="group bg-white dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-800 hover:border-indigo-300 dark:hover:border-indigo-700 hover:shadow-md transition-all cursor-pointer relative overflow-hidden"
                >
                  <div className="flex justify-between items-start relative z-10">
                    <div className="flex gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {sealed && (
                            <span className="text-[10px] font-bold px-2 py-0.5 rounded border border-amber-200 bg-amber-50 text-amber-700 flex items-center gap-1">
                              <Lock size={8} /> TERTUTUP
                            </span>
                          )}
                        </div>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-lg font-bold text-slate-900 dark:text-white font-mono">
                        IDR {(tender.totalValue / 1000000).toFixed(0)} M
                      </p>
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
                <p className="text-xs font-bold text-slate-500 uppercase mb-1">
                  Total HPS (Owner Estimate)
                </p>
                <p className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">
                  IDR {selectedTender.totalValue.toLocaleString()}
                </p>
              </div>
              <div className="p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 flex flex-col justify-center">
                ...
                <div className="w-full bg-slate-100 h-1.5 rounded-full mt-2">
                  <div
                    ...
                    style={{
                      width: `${(selectedTender.bids?.length || 0) * 20}%`,
                    }}
                  ></div>
                </div>
              </div>
            ...
            {isTenderSealed(selectedTender) && (
              <div className="p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800 rounded-xl flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-amber-900 dark:text-amber-100">
                      Dokumen Penawaran Tertutup
                    </h4>
                    <p className="text-xs text-amber-700 dark:text-amber-300">
                      Tanggal Buka: {selectedTender.bidOpeningDate}
                    </p>
                  </div>
                </div>
                {canOpenBids(selectedTender.bidOpeningDate) ? (
                  ...
                  <button
                    disabled
                    className="px-3 py-1.5 bg-slate-100 text-slate-400 font-bold rounded-lg text-xs flex items-center gap-2 cursor-not-allowed"
                  >
                    <Lock size={12} /> Terkunci hingga{" "}
                    {selectedTender.bidOpeningDate}
                  </button>
                )}
              </div>
            )}
              ...
              <div className="flex justify-between items-center mb-4">
                <div className="flex gap-2 bg-slate-100 dark:bg-slate-800 p-1 rounded-lg">
                  <button
                    ...
                    className={`p-1.5 rounded-md ${
                      viewMode === "list"
                        ? "bg-white shadow-sm dark:bg-slate-700 text-indigo-600"
                        : "text-slate-400"
                    }`}
                  >
                    <List size={16} />
                  ...
                  <button
                    ...
                    disabled={isTenderSealed(selectedTender)}
                    className={`p-1.5 rounded-md ${
                      viewMode === "matrix"
                        ? "bg-white shadow-sm dark:bg-slate-700 text-indigo-600"
                        : "text-slate-400"
                ...
                {selectedTender.bids &&
                  ...
                  !isTenderSealed(selectedTender) && (
                    ...
                    >
                      <BrainCircuit size={14} /> AI Evaluasi
                    </button>
                  )}
              </div>
                  ...
                  <div className="space-y-3">
                    {selectedTender.bids.map((bid, i) => {
                      ...
                      return (
                        <div
                          key={i}
                          className="flex justify-between items-center p-4 border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-900 hover:shadow-sm transition-shadow"
                        >
                          <div>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              {sealed ? (
                                ...
                                >
                                  Nilai Teknis: {bid.complianceScore}
                                </span>
                              )}
                            ...
                            {selectedTender.status === "Published" &&
                              !sealed && (
                                <button
                                  ...
                                  className="p-2 bg-slate-900 text-white rounded-lg hover:bg-slate-700 transition-colors tooltip"
                                  title="Tunjuk Pemenang"
                                >
                                  <Trophy size={16} />
                                </button>
                              )}
                    ...
                    <table className="w-full text-sm text-left border-collapse">
                      <thead>
                        <tr className="bg-slate-50 dark:bg-slate-800">
                          <th className="p-3 border dark:border-slate-700 text-xs text-slate-500 uppercase">
                            Kriteria
                          </th>
                          {selectedTender.bids.map((bid, i) => (
                            <th
                              key={i}
                              className="p-3 border dark:border-slate-700 font-bold text-slate-800 dark:text-white"
                            >
                              {bid.vendorName}
                            </th>
                          ))}
                      ...
                      <tbody>
                        <tr>
                          <td className="p-3 border dark:border-slate-700 font-bold text-slate-600">
                            Harga Penawaran
                          </td>
                          {selectedTender.bids.map((bid, i) => (
                            <td
                              key={i}
                              className="p-3 border dark:border-slate-700 font-mono text-slate-900 dark:text-white"
                            >
                              {isTenderSealed(selectedTender)
                                ? "**************"
                                : sealBid(bid.bidAmount, false)}
                        ...
                        <tr>
                          ...
                          {selectedTender.bids.map((bid, i) => (
                            <td
                              key={i}
                              className="p-3 border dark:border-slate-700"
                            >
                              <span
                        ...
                        <tr>
                          ...
                          {selectedTender.bids.map((bid, i) => (
                            <td
                              key={i}
                              className="p-3 border dark:border-slate-700 text-slate-600"
                            >
                              {60 + i * 5}% (Verified)
                            </td>
                          ))}
                          ...
                          {selectedTender.bids.map((bid, i) => (
                            ...
                            >
                              <button
                                ...
                                className="w-full py-2 bg-indigo-600 text-white rounded text-xs font-bold hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed"
                              >
                                Pilih Pemenang
                              </button>
                            </td>
          ...
          <div className="flex gap-4 mb-6 border-b border-slate-100 dark:border-slate-800 pb-4">
            <div
              className={`flex items-center gap-2 text-sm font-bold ${
                wizardStep === 1 ? "text-indigo-600" : "text-slate-400"
              }`}
            >
              <div
            ...
            <div
              className={`flex items-center gap-2 text-sm font-bold ${
                wizardStep === 2 ? "text-indigo-600" : "text-slate-400"
              }`}
            >
              <div
                className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${
              ...
              <div className="space-y-4">
                <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                  <p className="text-xs text-slate-500 mb-3 font-medium">
                    Permintaan Disetujui (Available)
                  </p>
                  {availableRequests.length === 0 ? (
                    <p className="text-center text-slate-400 text-sm italic py-4">
                      Tidak ada permintaan yang siap untuk tender.
                    </p>
                  ) : (
                ...
                <div className="flex justify-end">
                  <button
                    ...
                    className="px-6 py-2.5 bg-indigo-600 text-white rounded-lg font-bold text-sm disabled:opacity-50 disabled:cursor-not-allowed hover:bg-indigo-700 flex items-center gap-2"
                  >
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-bold text-slate-900 dark:text-white outline-none focus:border-indigo-500 dark:focus:border-indigo-400"
                    placeholder="e.g., Pengadaan Rig Tahunan"
                  />
                </div>

                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">
                    Tanggal Pembukaan Sampul
                  </label>
                  <input
                    type="date"
                    value={bidOpeningDate}
                    onChange={(e) => setBidOpeningDate(e.target.value)}
                    className="w-full p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-slate-900 dark:text-white outline-none focus:border-indigo-500 dark:focus:border-indigo-400"
                  />
                  <p className="text-[10px] text-slate-400 dark:text-slate-500">
                    Dokumen penawaran akan terenkripsi hingga tanggal ini.
                  </p>
                </div>
                ...
                <div className="space-y-2 relative">
                  ...
                  <textarea
                    ...
                    className="w-full p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-slate-900 dark:text-white outline-none focus:border-indigo-500 dark:focus:border-indigo-400 min-h-[100px]"
                    placeholder="Deskripsi detil pekerjaan..."
                  />
                  <button
                    onClick={generateSOW}
                    className="absolute right-2 top-8 p-1.5 text-indigo-600 hover:bg-indigo-50 rounded-md"
                ...
                <div className="bg-slate-50 dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 flex justify-between items-center">
                  ...
                  <span className="text-xl font-bold text-slate-900 dark:text-white font-mono">
                    IDR {calculateTotalBudget().toLocaleString()}
                  </span>
                </div>
                ...
                <div className="flex justify-between pt-4">
                  ...
                  <button
                    ...
                    className="px-6 py-2.5 bg-indigo-600 text-white rounded-lg font-bold text-sm hover:bg-indigo-700 shadow-sm flex items-center gap-2"
                  >
          ...
          {isGenerating ? (
            <div className="py-12 flex flex-col items-center">
              ...
              <p className="text-slate-500 font-medium">
                Menganalisis proposal teknis & komersial...
              </p>
            </div>
          ) : (
            <div className="whitespace-pre-line leading-relaxed text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
              {aiContent}
            </div>
          )}
        </div>
