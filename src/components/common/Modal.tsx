...
const Modal: React.FC<ModalProps> = ({
  ...
  return createPortal(
    ...
    >
      ...
      <div
        className={`relative bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full ${
          children ? "transition-all" : ""
        } ${
          maxWidth || "max-w-2xl"
        } max-h-[90vh] overflow-hidden flex flex-col border border-slate-200 dark:border-slate-800 transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] transform ${
      ...
      >
        <div className="sticky top-0 bg-white dark:bg-slate-900 z-10 px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center transition-colors shrink-0">
          <h3 className="text-sm font-bold text-slate-900 dark:text-white line-clamp-1 pr-4 uppercase tracking-wide">
            {title}
          </h3>
          <button
            onClick={onClose}
        ...
        <div
          className={`${
            noPadding ? "" : "p-6"
          } text-slate-800 dark:text-slate-200 overflow-y-auto custom-scrollbar`}
        >
          {children}
        </div>
      </div>
